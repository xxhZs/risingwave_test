...
use risingwave_pb::common::{ActorInfo, HostAddress, ParallelUnit, ParallelUnitMapping, WorkerType};
...
use risingwave_pb::stream_plan::{ActorMapping, DispatcherType, StreamActor, StreamNode};
use risingwave_pb::stream_service::{
    BroadcastActorInfoTableRequest, BuildActorsRequest, HangingChannel, UpdateActorsRequest,
};
        ...
        S: MetaStore,
{
    ...
    pub async fn migrate_actors(&self, table_id: &TableId, actors: HashMap<ActorId, WorkerId>) -> Result<()> {
        ...
        // let chain_actor_ids = HashSet::from(table_fragments.chain_actor_ids().into_iter().as_ref());
        //
        // for actor_id in &actor_ids {
        //     if chain_actor_ids.contains(actor_id) {
        //         return Err(internal_error("chain actor does not support migrate"));
        //     }
        // }

        for actor_id in &actor_ids {
            Self::resolv_dependent_actors(*actor_id, &actor_map, &mut downstream_actor_map, &mut upstream_actor_map);
        }

        let mut in_degree_map: HashMap<ActorId, u32> = actor_ids.iter().map(|actor_id| (*actor_id, 0)).collect();
        for actor_id in &actor_ids {
        ...
        for root in roots {
            if let Some(upstreams) = upstream_actor_map.get(&root) {
                if let Some(actor_status) = table_fragments.actor_status.get(&root) {
                    let actor_status = actor_status.parallel_unit.as_ref().unwrap().worker_node_id;
                    let worker = locations.worker_locations.get(&actor_status).unwrap();
        ...
        // for (a, b) in node_hanging_channels {
        //     println!("worker {}", a);
        //
        //     for c in b {
        //         println!("\tchannel {:?}", c);
